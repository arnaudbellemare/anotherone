...
from arch import arch_model # Ensure arch is imported for arch_model
from sklearn.linear_model import LinearRegression, Ridge # Ensure these are imported
...
def get_pretty_factor_name(factor_name: str) -> str:
    """
    Returns the human-readable "pretty" name for a given factor.

    Args:
        factor_name (str): The technical name of the factor (e.g., 'LogMktCap', 'ROE').

    Returns:
        str: The human-readable name if found in the METRIC_NAME_MAP,
             otherwise returns the original factor_name.
    """
    return METRIC_NAME_MAP.get(factor_name, factor_name)
################################################################################
...
# --- Stock Data Fetching Functions (ADD THESE) ---

@lru_cache(maxsize=None) # Cache individual stock history calls
def fetch_stock_history(ticker, period="5y"):
    """Fetches historical data for a single stock ticker."""
    history = yf.Ticker(ticker).history(period=period, auto_adjust=True, interval="1d")
    if history.empty or 'Close' not in history.columns:
        raise ValueError(f"No valid data for stock {ticker}")
    history.index = history.index.tz_localize(None) # Remove timezone for consistency
    history.dropna(subset=['Close'], inplace=True)
    if history['Close'].eq(0).any():
        raise ValueError(f"Zero Close prices found for stock {ticker}")
    return history

@st.cache_data(ttl=86400) # Cache the entire dictionary for one day
def fetch_all_stock_histories(_stock_tickers, period="5y"):
    """
    Fetches historical price data for a list of stock tickers in parallel.
    This is the missing function that caused the NameError.
    """
    stock_histories = {}
    with ThreadPoolExecutor(max_workers=20) as executor:
        # Create a dictionary mapping futures to tickers
        future_to_stock = {executor.submit(fetch_stock_history, ticker, period): ticker for ticker in _stock_tickers}
        
        # Use tqdm to show progress
        for future in tqdm(as_completed(future_to_stock), total=len(_stock_tickers), desc="Fetching Individual Stock Histories"):
            ticker = future_to_stock[future]
            try:
                # Get the result from the future
                stock_histories[ticker] = future.result()
            except Exception as e:
                # Log an error if a ticker fails, but continue with others
...
def fetch_ticker_data(ticker_symbol):
    """
    Fetches ticker data using a cached session with a browser user-agent to improve reliability.
    """
    # Use a cached session to avoid re-downloading and reduce rate-limiting issues.
    # This creates a 'yfinance.cache' file in your directory.
    session = requests_cache.CachedSession('yfinance.cache')
    session.headers['User-agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
    
    ticker = yf.Ticker(ticker_symbol, session=session) # Pass the robust session to yfinance
    
    # Fetch data with a longer period initially to ensure enough data for all calculations
    history = ticker.history(period="7y", auto_adjust=True, interval="1d").tz_localize(None)
    
    # --- CRITICAL ROBUSTNESS CHECK ---
    # If history is too short after a successful download, something is wrong.
    if history.empty or len(history) < 252: # Require at least 1 year of data to be considered valid
        raise IOError(f"Insufficient historical data for {ticker_symbol}. Received {len(history)} rows.")
    ...
    try:
        ...
        for period in periods:
            if period is not None and len(history) > period:
                ...
                returns[f"Return_{period}d"] = np.nan # <--- COLON REMOVED, BUG FIXED
        return returns
    except Exception as e:
...
def calculate_returns_cached(ticker, periods_tuple):
    ...
    try:
        ...
        logging.error(f"Error calculating returns for {ticker} for periods {periods_tuple}: {e}")
        return {f"Return_{p}d": np.nan for p in periods}
# --- New Helper functions for Institutional Factors ---
...
def process_single_ticker(ticker_symbol, etf_histories, sector_etf_map):
    ...
    try:
       ...
       # --- Final Data Preparation ---
        ...
        # --- FIX: RETURN THE DICTIONARY, NOT A LIST ---
        ...
        # --- FIX: RETURN THE DICTIONARY ON ERROR TOO ---
...
# =================================================================================
# --- THIS IS THE CORRECT AND COMPLETE process_tickers FUNCTION ---
# =================================================================================

@st.cache_data
def process_tickers(_tickers, _etf_histories, _sector_etf_map, _all_histories):
    """
    Processes tickers using pre-fetched historical data. This is the efficient version.
    
    Args:
        _tickers (list): List of ticker symbols to process.
        _etf_histories (dict): Dictionary of pre-fetched ETF history DataFrames.
        _sector_etf_map (dict): Mapping of sectors to ETFs.
        _all_histories (dict): Dictionary of pre-fetched stock history DataFrames from fetch_all_histories_robustly.
    """
    results, returns_dict, failed_tickers = [], {}, []

    # Stage 1: Fetch all non-historical data (info, financials, etc.) in parallel.
    # This is done first because it's a common point of failure (e.g., rate limiting).
    all_other_data = {}
    with ThreadPoolExecutor(max_workers=10) as executor:
        def fetch_non_history_data(ticker_symbol):
            """Helper to fetch all yfinance objects for a single ticker."""
            ticker = yf.Ticker(ticker_symbol)
            # The .info can be slow, so it's the main target for parallelization.
        ...
        for future in tqdm(as_completed(future_to_fetch), total=len(_tickers), desc="Fetching Financials & Info"):
            ...
            try:
                # Store all fetched data in a dictionary keyed by the ticker
                all_other_data[ticker_symbol] = future.result()
            except Exception as e:
                logging.error(f"Failed to fetch non-history data for {ticker_symbol}: {e}")
                failed_tickers.append(ticker_symbol)

    # Stage 2: Process all the gathered data in parallel. This is CPU-bound.
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_process = {}
        for ticker_symbol in _tickers:
            # Skip tickers that failed the previous fetching stage or have no history
            if ticker_symbol in failed_tickers or ticker_symbol not in _all_histories:
                continue
            
            # Retrieve the pre-fetched data for this specific ticker
            ...
            # Submit the processing task. This calls the 12-argument version of process_single_ticker.
        ...
        for future in tqdm(as_completed(future_to_process), total=len(future_to_process), desc="Calculating All Metrics"):
            ticker = future_to_process[future]
            try:
                # The process_single_ticker function should return a dictionary of metrics and a Series of returns
                ...
                logging.error(f"Failed during metric calculation for {ticker}: {e}")
                failed_tickers.append(ticker)
    
    # Final DataFrame creation and cleaning
    if not results:
    ...
    # Robustly clean and fill the final DataFrame
